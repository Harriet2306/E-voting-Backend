...
exports.getBallot = async (req, res) => {
  try {
    ...
    // FILTER BY FACULTY ACCESS CONTROL
    const { getFacultyFromProgram } = require('../utils/facultyUtils');
    const voterFaculty = getFacultyFromProgram(ballot.voter.program);

    console.log(`Access Control: Voter Program '${ballot.voter.program}' mapped to '${voterFaculty}'`);

    const positions = openPositions.filter(pos => {
      // If position has no faculty (null), it's GLOBAL (e.g. Guild President) -> SHOW
      if (!pos.faculty) return true;

      // If position has faculty, it must match voter's faculty -> SHOW
      // Normalize comparison
      return pos.faculty.toLowerCase() === voterFaculty.toLowerCase();
    });

    console.log(`Backend getBallot - found ${positions.length} accessible positions out of ${openPositions.length} open based on faculty '${voterFaculty}'`);
