...
exports.submitNomination = async (req, res) => {
  try {
    ...
    // Photo is required, manifesto is optional (can be uploaded later)
    if (!req.files?.photo?.[0]) {
      return res.status(400).json({ error: 'Photo is required' });
    }

    // Get file URLs from uploaded files
    // Manifesto is optional - can be null initially
    const manifestoUrl = req.files?.manifesto?.[0] ? `/uploads/${req.files.manifesto[0].filename}` : null;
...
/**
 * Upload or update manifesto for an existing nomination
 * Allows candidates to upload manifesto after initial nomination submission
 */
exports.uploadManifesto = async (req, res) => {
  try {
    ...
    // Verify ownership - only the candidate who submitted can update
    if (candidate.userId !== userId) {
      return res.status(403).json({ error: 'You can only update your own nominations' });
    }

    // Get file URL from uploaded file
    const manifestoUrl = `/uploads/${req.files.manifesto[0].filename}`;

    // Update the candidate with new manifesto
    ...
    res.status(500).json({ error: 'Failed to upload manifesto' });
  }
};
